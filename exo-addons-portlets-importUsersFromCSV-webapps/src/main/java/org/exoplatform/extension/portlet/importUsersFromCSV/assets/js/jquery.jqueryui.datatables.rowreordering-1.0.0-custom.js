    ...
    $.fn.rowReordering = function (options) {
        ...
        function fnMoveRows(sSelector, iCurrentPosition, iNewPosition, sDirection, id, sGroup) {
            ...
            $(oTable.fnGetNodes()).each(function () {
                if (sGroup != "" && $(this).attr("dataAccess-group") != sGroup)
                    return;
                var tr = this;
            ...
            if (oSettings.oFeatures.bServerSide === false) {
                var scrollTop = $(oSettings.nTable.parentNode).scrollTop(); // CUSTOM
                var before = oSettings._iDisplayStart;
                oSettings.oApi._fnReDraw(oSettings);
                //iDisplayStart has been reset to zero - so lets change it back
                oSettings._iDisplayStart = before;
                oSettings.oApi._fnCalculateEnd(oSettings);
                $(oSettings.nTable.parentNode).scrollTop(scrollTop); // CUSTOM
            }
        ...
        var defaults = {
            ...
            fnAlert: _fnAlert,
            iLogLevel: 1,
            sDataGroupAttribute: "dataAccess-group",
        ...
        return this.each(function () {
            ...
            $("tbody", oTable).sortable({
                ...
                // CUSTOM
                axis: "y",
                containment: ".dataTables_scrollBody", // TODO Detect is scrolling
                handle: "> td:nth-child(" + (properties.iIndexColumn + 1) + ")",
                scrollSpeed: 10,
                start: function(event, ui)
                {
                    ui.item.find("> td").each(function(tableDataIndex, tableData)
                    {
                        var $tableHead = $(oTable.fnSettings().nTHead).find("> tr > th:nth-child(" + (tableDataIndex + 1) + ")");
                        $(tableData).width($tableHead.outerWidth());
                    });
                },
                tolerance: "pointer",
                // CUSTOM
